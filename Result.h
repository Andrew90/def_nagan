...
public:
	...
	bool popBrack, prodBrack, thicknessBrack;
	SummaryResult();			//конструктор
	~SummaryResult();			// деструктор
	void ComputeZonesData(Result &_pp, Result &_pr, ThicknessResult &_th);	// считает суммарный результат по всем модулям, заполянет массив zone_data
	TColor ZoneColor(double meas);      // возвращает цвет зоны
