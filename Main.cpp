...
void __fastcall TMainForm::FormCreate(TObject *Sender)
{
	...
	Lir::Instance().Init(digital->AdvCards[0], digital->AdvCards[1]);
   //	Lir::Instance().Start();
...
// ждем получения группы прочности
		...
		bool &isEmpty = Lir::Instance().sGUnit.unitEmpty;
	//	while(isEmpty)Sleep(100);
		/*
		...
		*/
		{
		  ...
		  //	vector<int>buffer = PrepareSG::Instance().Do(l1->Base, l1->Index, l1->timeOffs);                      //Берем вектор
			vector<int>buffer;
			l1->PrepareSGData(false, buffer);		//пихаем в него полученный сигнал
...
// ждем результата от толщины, если она работала
		...
		{
			if(digital->WaitForInSignal("Результат",360000) )
			{
				if (!SystemConst::IsOnline)
				{
...
void __fastcall TMainForm::LirOptionsMenuItemClick(TObject *Sender)
{
