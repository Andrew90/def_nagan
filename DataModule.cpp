...
int Customer()
{
		...
		DM->Find->SQL->Clear();
		DM->Find->SQL->Add("Select CustomerID  from dbo.Packets ORDER BY DateTimeCreated DESC");
		DM->Find->ExecSQL();
		DM->Find->Active=true;
		int cust=DM->Find->FieldByName("CustomerID")->AsInteger;
		return cust;
}
					...
					int TypeSize,bool isEtalon ) //
{
	...
	else
	{
		...
		TubeTable->FieldByName("MinThickness")->AsFloat = thick__(TypeSize);
		TubeTable->Post();
	}
		...
		TubeTable->FieldByName("TypeSize")->AsInteger=TypeSize;
			TubeTable->FieldByName("CustomerID")->AsInteger=cust;
		TubeTable->FieldByName("Length")->AsFloat= 200.0 * lengthArray;
		if("Б" == Result)
		{
		   if(MainForm->sum.popBrack)TubeTable->FieldByName("CrossResult")->Value = "Б";
		   if(MainForm->sum.prodBrack)TubeTable->FieldByName("LineResult")->Value = "Б";
		   if(MainForm->sum.thicknessBrack)TubeTable->FieldByName("ThicknessResult")->Value = "Б";
        }
