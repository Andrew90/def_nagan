...
struct LirChange
{
	...
	unsigned lirOffsetUp;
	unsigned lirOffsetDown;
	double samplesPerZone0;
	double samplesPerZone1;
...
struct SGUnit
{
	...
	unsigned &bit2;
	unsigned bitTimeUp;
	unsigned bitTimeDown;
	bool unitEmpty;
	void (SGUnit::*ptr)();//указатель на текущий метод
	SGUnit(Lir &o);
	void Start();//начало цикла
	void StartUp();
	void StartDown();
...
public:
	~Lir(void);
	void Init(Advantech1730 *, Advantech1730 *);
	void Destroy();
	void Start(bool isThick, bool isLong);
	void Prepare();   //
	void ChangeSamplesLir(int, int);
	void GetSamplesLir(int&, int &);
