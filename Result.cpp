...
// возвращает цвет зоны в зависимости от толщины в этой зоне
   //	if ( meas == 8.0 )
	  //	return Dead;
	//else
	if (meas >= ThicknessResult::borders[borders.get_high()])
		return Good;
...
//---------------------------------------------------------------------------
void SummaryResult::ComputeZonesData(Result &_pp, Result &_pr, ThicknessResult &_th)
{
...
// 1 - годно, 0 - брак, 2 - 2 класс, 3 - 3 класс...
    popBrack = prodBrack = thicknessBrack = false;
	for (int i = 0; i < SummaryResult::zones; i++)
	{
				...
				( (th) && _th.zone_data[i] < _th.borders[0] ) )
		{
			...
			if(_pp.zone_data[i] >= _pp.borders[0]) popBrack = true;
		if((lin) && _pr.zone_data[i] >= _pr.borders[0] ) prodBrack = true;
		if((th) && _th.zone_data[i] < _th.borders[0]  )  thicknessBrack = true;
		}
